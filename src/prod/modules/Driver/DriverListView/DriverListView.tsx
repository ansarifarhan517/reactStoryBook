...
const DriverListView = ({ ngStateRouter }: IDriverListViewViewProps) => {
  ...
  const structure = useTypedSelector(state => state.driver.listView.structure)
  const columnsSelector = useTypedSelector(state => state.driver.listView.structure.columns)
  const rowsSelector = useTypedSelector(state => state.driver.listView.data.results)
  const totalRowsSelector = useTypedSelector(state => state.driver.listView.data.totalCount)
  const pageLabels = useTypedSelector(state => state.pageLabels.driver)
  const actionBarButtons = useTypedSelector(state => state.driver.listView.structure.buttons)
  ...
  const loading = useTypedSelector(state => state.driver.listView.loading.listView)
  const inputVal = useTypedSelector(state => state.driver.listView.inputVal)
  ...
  useEffect(() => {
    ...
    if (Object.entries(columnsSelector).length === 0) {
      dispatch({ type: '@@driverListView/FETCH_STRUCTURE' })
    }
    fetchVehicleTypes()
  ...
  const handleFetchData = React.useCallback(({ pageSize, pageNumber, sortOptions, filterOptions, apis }: IFetchDataOptions) => {
    ...
    dispatch({
      type: '@@driverListView/SET_LOADING',
      payload: { listView: true, inputVal: true }
    })
    setFetchOptions({ pageSize, pageNumber, sortOptions, filterOptions, apis })
    dispatch({
      type: '@@driverListView/FETCH_DATA',
      payload: {
