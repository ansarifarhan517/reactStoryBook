...
import { Modal, ModalHeader, Accordion, Box, IconButton, useToast, IListViewColumn, ISelectedRows, IFetchDataOptions, ListView, ISortOptions, FontIcon, IListViewRow } from 'ui-library';
...
import { ExceptionListviewContainer, ExceptionListviewFooter, ModalHeaderWithIcon, ReloadRibbon } from "./OutscanStyledComponent";
import { isEmpty, usePagination } from "../utils";
import { IExceptionData, IExceptionRecord } from "../OutscanOrderManifest.models";
import { usePageVisibility } from "../../../utils/usePageVisibility";

interface IExceptionListModalProps {
    isOpen: boolean
    onClose: React.Dispatch<SetStateAction<boolean>>;
    scanSelectedType: Record<string, any>;
...
const ExceptionListModal = ({ isOpen, onClose, scanSelectedType, selectedAccordianType, continueScanning, scanApiPayload, setExceptionList }: IExceptionListModalProps) => {
    ...
    const structure = useTypedSelector(state => state.outscan.listview.exceptions.structure);
    const columnsSelector = useTypedSelector(state => state.outscan.listview.exceptions.structure.columns);
    const loading = useTypedSelector(state => state.outscan.listview.exceptions.loading);
    const rowsSelector = useTypedSelector(state => state.outscan.listview.exceptions.data.results);
    /** State */
    ...
    useEffect(() => {
        ...
        dispatch({ type: '@@outscanOrderManifest/FETCH_EXCEPTION_LISTVIEW_STRUCTURE' });
    }, []);
    // resetting ref value on toggle
    useEffect(() => {
    ...
    const fetchExceptionList = async () => {
        try {
            const { data: data } = await axios.post(apiMappings.outscan.form.outscan, scanApiPayload);
    ...
    const paginateData = (data: IExceptionRecord[], pageNumber: number, pagSize: number) => {
    ...
    const onSaveColumnPreferences = React.useCallback(async (visibleColumns: Record<string, ColumnInstance<IListViewColumn>>) => {
        ...
        try {
            const { data: { message } } = await axios.put(apiMappings.outscan.listview.exceptions.structure, payload)
        ...
        <Modal open={isOpen} onToggle={() => onClose(false)} size='md' width="1140px">
            {{
                ...
                <ExceptionListviewFooter>
                       <IconButton id='ExceptionList-Modal-button-ContinueScanning' iconVariant='scan-continue' disabled={rowsSelector?.some((row: IExceptionRecord) => row.exceptionType === "Restrictive")} primary onClick={() => continueScanning(scanSelectedType, selectedAccordianType)}>{dynamicLabels.continueScanning}</IconButton>
                       <IconButton id='ExceptionList-Modal-button-Cancel' iconVariant='scan-cancel'  onClick={() => onClose(false)} >{dynamicLabels.cancelScanning}</IconButton>
                </ExceptionListviewFooter>),
            }}
        </Modal>
    )
