...
import { IActivationConfirmation, IActivationModalButtonGroup } from '../FleetTypeListView.models'
        ...
        {{
            ...
            footer: (
                <Box horizontalSpacing='10px' display='flex' justifyContent='flex-end' p='15px'>
                    {
                        footerButtonGroup.map((button: IActivationModalButtonGroup,index:number) => {
                            return !!button.isVisible &&
                                <IconButton key={button.label + index} iconVariant={button.iconVariant} primary={button.primary} onClick={button.onClick}>{button.label}</IconButton>
                        })
                    }
                </Box>
            )
