...
import useClientProperties from '../ClientProperties/useClientProperties';
...
const ThirdElement = ({ value, setValue, operationType, fieldType, columnName ,customSectionName}: IThirdElementProps) => {
    const [options, setOptions] = useState<any[]>([]);
    const [valueArray, setValueArray] = useState<any>([])
    const dynamicLabels = useTypedSelector(state => state.dynamicLabels)
    const columnsSelector = useTypedSelector(state => state.advanceFilter.columnsSelector);
    const sectionName = useTypedSelector(state => state.advanceFilter.sectionName) || customSectionName || '';
    ...
    //         return Object.keys(columnsSelector[cName].dropdownValues).map((key) => ({ value: key, label: key }))
    //     }else {
    //         return dropdownMap[cName] && dropdownMap[cName].length > 0 ? dropdownMap[cName] : []
    //     }
    // };

    const fetchDropdownOptions = async (columnName: any) => {
        const getter = DROPDOWN_FILTER_OPTIONS_MAPPING?.[sectionName]?.[columnName]
        if (columnsSelector[columnName]?.dropdownValues) {
            const dropdownValues = Object.keys(columnsSelector[columnName].dropdownValues).map((key) => ({ value: key, label: key }))
            ...
            console.log(columnName, options)
        }
    }


    useEffect(() => {
        if (Object.keys(columnsSelector).length > 0) {
            fetchDropdownOptions(columnName)
        }
        //setOptions(DropdownOptions(columnName, columnsSelector));
    }, [columnsSelector, columnName, dynamicLabels, sectionName]);

    useEffect(() => {
        if (typeof value === 'string') {
            if (fieldType === 'select' || fieldType === 'dropdown' || fieldType == 'skillsetfield') {
                // do the conversion
    ...
    if ((fieldType === 'text' && oType !== null) ||
        ...
        (fieldType === 'multiSelect' && oType !== 'filterOpEquals') ||
        (fieldType === 'multiSelect' && oType === 'filterOpNotIn') ||
        (fieldType === 'select' && oType === 'filterOpNotIn') ||
        (fieldType === 'dropdown' && oType === 'filterOpNotIn') || 
        (fieldType == 'skillsetfield' && oType === 'filterOpIn') ||
        (fieldType == 'skillsetfield' && oType === 'filterOpNotIn')
    ) {
        const handleChange = (
            _event: any,
            _value: any,
        ...
        (fieldType === 'multiselect' && oType === 'filterOpEquals') ||
        (fieldType === 'multiSelect' && oType === 'filterOpEquals') || 
        (fieldType == 'skillsetfield' && oType !== null)
    ) {
        return (
            <AdvFilterDropdown
                ...
                {({ value, open, setOpen }: any) => (
                    <div onClick={() => setOpen(!open)}>
                        <TextInput
                            ...
                            value={value ? moment.utc(moment.utc(value, `HH:mm:ss`), `HH:mm:ss`).tz(clientProperties?.TIMEZONE
                                ?.propertyValue).format("HH:mm:ss") : ''}
                            read-only
                            fullWidth
                            onChange={() => { }}
                        />
        ...
        return (
            <DateRangePicker
                ...
                onApply={(date: any) => {
                    const from = moment(date[0], format).startOf('day').utc().format(`${format} HH:mm:ss`)
                    const to = moment(date[1], format).endOf('day').utc().format(`${format} HH:mm:ss`)
                    setValue(from + ',' + to)
                }}
                label='Date'
                variant='daterange'
                    ...
                    return (
                        <div onClick={() => setOpen(!open)}>
                            <TextInput
                                ...
                                placeholder=''
                                fullWidth
                                value={startDate ? `${moment(startDate).format(`${format}`)} - ${moment(endDate).format(`${format}`)}`
                                    : value}
                            />
                        </div>
                    )
