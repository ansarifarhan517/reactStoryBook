...
import { FormFieldWapper, CloneFieldsContainer } from "../../MobileRolesStyledComponents"
import useDynamicLabels from "../../../../common/DynamicLabels/useDynamicLabels";
import { IFormInputs } from "../../MobileRoles.models"
...
const CloneMobileTemplateModal = ({ isOpen, onClose, onSubmit }: ICloneMobileRoleProps) => {
    ...
    return (
        <Modal open={isOpen} onToggle={() => onClose(false)} size='md' width='600px'>
            {{
                header: <ModalHeader headerTitle={dynamicLabels.cloneMobileRole} handleClose={() => onClose(false)} />,
                            ...
                            <div key={sectionName}>
                                <Grid container spacing="0px">
                                    {Object.keys(structure[sectionName]).map((fieldName) => {
                                        if (fieldName === 'orgRoleName' || fieldName === 'orgRoleDescription') {
                ...
                footer: <Box horizontalSpacing='10px' display='flex' justifyContent='flex-end' p='15px'>
                    <IconButton primary iconVariant='icomoon-save'
