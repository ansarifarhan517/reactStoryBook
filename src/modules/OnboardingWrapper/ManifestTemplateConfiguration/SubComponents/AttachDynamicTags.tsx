...
import CustomFieldsGrouppedDropDown from "../utils/CustomFieldsGroupedDropdown";
...
const AttachDynamicTags = ({
  ...
  const isTagCategoryEmpty = useTypedSelector(
    (state) => state.manifestTemplateConfiguration.tags
  );
...
function InlinePopupContent({ onSelect }: IInlinePopupContentProps) {
  const { propertyType, tags } = useTypedSelector(
    (state) => state.manifestTemplateConfiguration
  );
  const dynamicLabels = useDynamicLabels(
    DYNAMIC_LABELS_MAPPING.awbLabelConfiguration
  );
  ...
  const categories = React.useMemo(() => {
    ...
    if (propertyType.propertyValue !== "CRATEITEM") {
  ...
  const data = React.useMemo(() => {
    return tags.dynamicTagKeys.map(
      ({ dynamicTagKey, tagGroupId, dynamicTagLabelValue, isCustomField }) => ({
        id:
          dynamicTagKey == "#@--statusCd--@#" ||
          dynamicTagKey == "#@--orderNo--@#"
            ? dynamicTagKey + String(tagGroupId)
            : dynamicTagKey,
        label: isCustomField ? (
          <CustomFieldsGrouppedDropDown
            customLabel={dynamicTagLabelValue}
            customText={dynamicLabels.md_custom_label}
          />
        ) : (
          dynamicTagLabelValue
        ),
        value:
          dynamicTagKey == "#@--statusCd--@#" ||
          dynamicTagKey == "#@--orderNo--@#"
            ? dynamicTagKey + String(tagGroupId)
            : dynamicTagKey,
        categoryId: isCustomField
          ? String(tags.tagGroupMappingList.length + 1)
          : String(tagGroupId),
